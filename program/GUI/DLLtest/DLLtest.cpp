...
/* 主程序开始 */
void __stdcall solve()
{
	bool nxtlev[symnum];
	//strcpy(fname,"in.txt");
	fileName = fileDir + "in.txt";
	//printf("hhhh\n");
	...
	rewind(fin);*/
	//fopen_s(&foutput, "foutput.txt", "w");
	fileName = fileDir + "foutput.txt";
	...
	//fopen_s(&ftable, "ftable.txt", "w");
	fileName = fileDir + "ftable.txt";
	ftable = fopen(fileName.c_str(), "w");
	if ((ftable) == NULL)
	{
	...
	/*
	fileName = fileDir + "finput.txt";
	finput = fopen(fileName.c_str(), "w");
	if ((finput) == NULL)
	{
	...
	if (err == 0)
	{
		printf("\n===Parsing success!===\n");
		fprintf(foutput, "\n===Parsing success!===\n");
		//fopen_s(&fcode, "fcode.txt", "w");
		fileName = fileDir + "fcode.txt";
		fcode = fopen(fileName.c_str(), "w");
		//fcode = fopen("D:\\Program\\small_c_final\\fcode.txt", "w");
		if (fcode == NULL)
		{
			printf("Can't open fcode.txt file!\n");
			exit(1);
		}
		//fopen_s(&fresult, "fresult.txt", "w");
		fileName = fileDir + "fresult.txt";
		fresult = fopen(fileName.c_str(), "w");
		if ((fresult) == NULL)
		{
...
void block(int lev, int tx, bool* fsys)
{
	...
	if (tableswitch)		/* 输出符号表 */
	{
		for (i = 1; i <= tx; i++)
		{
			...
			{
			case constant:
				fprintf(ftable, "%5d const %10s| ", i, table[i].name);
				fprintf(ftable, "type=%5s| ", table[i].type == 1 ? "int" : "bool");
				if (table[i].type == 1)
					fprintf(ftable, "val=%5d| ", table[i].val);
				else fprintf(ftable, "val=%5s| ", table[i].bl ? "true" : "false");
				fprintf(ftable,"         | ");
				fprintf(ftable, "          | \n");
				break;
			case variable:
				fprintf(ftable, "%5d var   %10s| ", i, table[i].name);
				fprintf(ftable, "type=%5s| ", table[i].type == 1 ? "int" : "bool");
				fprintf(ftable, "         | ");
				//fprintf(ftable, "               ");
				fprintf(ftable, "lev=%5d| addr=%5d|\n", table[i].level, table[i].adr);
				break;
			case proc:
				fprintf(ftable, "%5d proc  %10s| ", i, table[i].name);
				fprintf(ftable, "          | ");
				fprintf(ftable, "         | ");
				fprintf(ftable, "lev=%5d| addr=%5d| size=%d\n", table[i].level, table[i].adr, table[i].size);
				break;
			}
		}
		fprintf(ftable, "\n");
		...
		case opr:	/* 数学、逻辑运算 */
			...
			case 16:/* 读入一个输入置于栈顶 */
				...
				if (i.l == 0) {
					...
					fprintf(fresult, "please input a number:");
					fscanf(finput, "%d", &(s[t]));
					printf("%d", s[t]);
					//fscanf(finput,"%d\n", &(s[t]));
					fprintf(fresult, "%d\n", s[t]);
					...
					printf("please input \"true\" or \"false\"(other input will be regard as \"false\")：\n");
					fprintf(fresult, "please input \"true\" or \"false\"(other input will be regard as \"false\")：\n");
					fscanf(finput,"%s\n", strtmp);
					fprintf(fresult, "%s\n", strtmp);
